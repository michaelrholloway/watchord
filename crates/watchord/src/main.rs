...
usage: watchord [--input <name>] [--skin push|plain] [--print | --json]
  ...
  --fake-rich       a scripted nine-chord session with every plain-skin
                    region populated, for a design screenshot
...
fn main() -> ExitCode {
    ...
    let is_fake =
        args.fake || args.fake_released || args.fake_fit || args.fake_nearest || args.fake_rich;
